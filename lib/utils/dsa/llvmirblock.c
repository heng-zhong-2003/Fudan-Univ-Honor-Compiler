...
/* Make a block list */
LL_blockList
LL_BlockList (LL_block head, LL_blockList tail)
{
  LL_blockList list = checked_malloc (sizeof *list);
  list->head = head;
  list->tail = tail;
  return list;
}
...
LL_blockList
LL_BlockSplice (LL_blockList a, LL_blockList b)
{
  LL_blockList p;
  if (a == NULL)
    return b;
...
/* given a list of instructions that's a basic block, i.e.,
   ...
   make a LL_block, which consists of instrs, start_label, jump_to_labels
*/

LL_block
LL_Block (LL_instrList instrs)
{
  LL_block b = checked_malloc (sizeof *b);
  LL_instrList last;
  b->instrs = instrs;
  // DBGPRT ("LL_Block instrs->head assem %s\n", instrs->head->u.LABEL.assem);
  if (instrs->head->kind == LL_LABEL)
    b->label = instrs->head->u.LABEL.label;
  else
    assert (0); // has to be a label!
  last = instrs;
  while (last->tail)
    last = last->tail;
  if (last->head->kind == LL_OPER)
    {
...
static LL_blockList global_bl;
static S_table block_env;

static LL_instrList getNext (LL_instrList epilog, bool optimize);

static LL_instrList
getLast (LL_instrList list)
// list must leng>=2. sen'd to last!
{
  LL_instrList last = list;
...
static void
trace (LL_instrList list, LL_instrList epilog, bool optimize)
{                                     // this list is a basic block
  LL_instrList last = getLast (list); // second to the last before the br/ret
  LL_instr lab = list->head;          // this must be a label instruction
  LL_instr s = last->tail->head;      // the last (last) stm (jump/cjump)
  // or a return
  S_enter (block_env, lab->u.LABEL.label, NULL); // mark it already traced!

  if (!optimize)
    { // just pick up the next block and continue
  ...
  */
  if (s->kind != LL_OPER)
    assert (-1); // can't be a label of move!
  ...
  if (labellist->tail)
    { // more than two labels! nothing to do again
      last->tail->tail = getNext (epilog, optimize);
      return;
    }
  ...
  LL_block target = (LL_block)S_look (block_env, nn);
  if (target)
    {
...
static LL_instrList
getNext (LL_instrList epilog, bool optimize)
{
  if (!global_bl)
    return epilog;

  LL_block b = global_bl->head;
 ...
 */
LL_instrList
LL_traceSchedule (LL_blockList bl, LL_instrList prolog, LL_instrList epilog,
                  bool optimize)
{
  LL_blockList blist;
  LL_instrList ll;
