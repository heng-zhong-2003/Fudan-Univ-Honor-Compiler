...
AS_instr
AS_Oper (string a, Temp_tempList d, Temp_tempList s, AS_targets j)
{
...
AS_instr
AS_Label (string a, Temp_label label)
{
  AS_instr p = (AS_instr)checked_malloc (sizeof *p);
  p->kind = I_LABEL;
  p->u.LABEL.assem = a;
...
AS_instrList
AS_InstrList (AS_instr head, AS_instrList tail)
{
...
AS_instrList
AS_splice (AS_instrList a, AS_instrList b)
{
  AS_instrList p;
  if (a == NULL)
    return b;
...
void
AS_format (char *result, string assem, Temp_tempList dst, Temp_tempList src,
           AS_targets jumps, Temp_map m)
{
...
void
AS_print (FILE *out, AS_instr i, Temp_map m)
{
  char r[200]; /* result */
  switch (i->kind)
    {
...
AS_proc
AS_Proc (string p, AS_instrList b, string e)
{
